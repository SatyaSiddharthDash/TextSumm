...
from textsumm.common.pytorch_utils import (
...
from textsumm.models.transformers.common import Transformer
    ...
    def collate(self, data, block_size, device, train_mode=True):
        ...
        Returns:
            namedtuple: a nametuple containing input ids, segment ids,
                ...
                in the target and target text.
        """
        data = [x for x in data if not len(
            x["src"]) == 0]  # remove empty_files
        if len(data) == 0:
            return None
        ...
        encoded_stories = torch.tensor(
            [
                fit_to_block_size(story, block_size,
                ...
                try:
                    ...
                    summary_lines_token_ids.append(
                        self.tokenizer.encode(
                            line, max_length=self.max_tgt_len)
                    )
                except:
                    print(line)
                    raise
...
class BertSumAbs(Transformer):
    ...
    def predict(
        ...
        for batch in tqdm(
            ...
            input = self.processor.get_inputs(
                batch, device, "bert", train_mode=False)
